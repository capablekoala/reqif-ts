...
export class ReqIFParser {
  ...
  static parseContent(
    ...
    if (rootElement.tagName !== "REQ-IF") {
      throw new ReqIFError(
        `Invalid ReqIF file: Root element is not REQ-IF but ${rootElement.tagName}`,
      );
    }
    ...
    const reqIfContentElement =
      coreContentElement.querySelector("REQ-IF-CONTENT");
    if (!reqIfContentElement) {
      throw new ReqIFError(
        "Invalid ReqIF file: Missing REQ-IF-CONTENT element",
      );
    }

    // Parse data types
    const dataTypes = Array.from(
      reqIfContentElement.querySelectorAll(
    ...
    const specObjectTypes = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-TYPES > SPEC-OBJECT-TYPE"),
    ).map((element) => {
    ...
    const specificationTypes = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-TYPES > SPECIFICATION-TYPE"),
    ).map((element) => {
      const specificationType = SpecificationTypeParser.parse(
        element as Element,
      );
    ...
    const specRelationTypes = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-TYPES > SPEC-RELATION-TYPE"),
    ).map((element) => {
    ...
    const relationGroupTypes = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-TYPES > RELATION-GROUP-TYPE"),
    ).map((element) => {
      const relationGroupType = RelationGroupTypeParser.parse(
        element as Element,
      );
    ...
    // Parse spec objects
    const specObjects = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-OBJECTS > SPEC-OBJECT"),
    ).map((element) => {
    ...
    const specifications = Array.from(
      reqIfContentElement.querySelectorAll("SPECIFICATIONS > SPECIFICATION"),
    ).map((element) => {
    ...
    const specRelations = Array.from(
      reqIfContentElement.querySelectorAll("SPEC-RELATIONS > SPEC-RELATION"),
    ).map((element) => {
...
export class ReqIFZParser {
   ...
   */
  static async parse(_filePath: string): Promise<any> {
