...
// ReqIF schema URL
const REQIF_SCHEMA_URL = 'http://www.omg.org/spec/ReqIF/20110401/reqif.xsd';
...
export class SchemaValidator {
  ...
  static validate(xmlDoc: Document): boolean {
    try {
      ...
      if (rootElement.tagName !== 'REQ-IF') {
        throw new ReqIFError(`Invalid root element: expected 'REQ-IF', got '${rootElement.tagName}'`);
      }
      ...
      for (const attr of requiredHeaderAttrs) {
        if (!header.hasAttribute(attr)) {
          throw new ReqIFError(`Missing required attribute in REQ-IF-HEADER: ${attr}`);
        }
      }
      
      // Check content
      const content = rootElement.querySelector('CORE-CONTENT > REQ-IF-CONTENT');
      if (!content) {
      ...
      if (error instanceof ReqIFError) {
        ...
        throw new ReqIFError(`Schema validation error: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
  ...
  private static validateReferences(xmlDoc: Document): void {
    ...
    const specRelationTypeRefs = xmlDoc.querySelectorAll('SPEC-RELATION-TYPE-REF');
    ...
    const specificationTypeRefs = xmlDoc.querySelectorAll('SPECIFICATION-TYPE-REF');
