...
import {
  ...
  serializeXML,
  parseXML,
} from "./helpers/xml-parser";
import { ReqIFBundle } from "./reqif-bundle";
import {
  ReqIFDataTypeKind,
  ReqIFAttributeValueKind,
} from "./models/reqif-types";
...
export class ReqIFUnparser {
  private static readonly REQIF_NAMESPACE =
  ...
  static unparse(reqIFBundle: ReqIFBundle): string {
    ...
    // Set namespaces
    reqIfElement.setAttributeNS(
      ReqIFUnparser.XMLNS_NAMESPACE,
      "xmlns",
      ReqIFUnparser.REQIF_NAMESPACE,
    );
    reqIfElement.setAttributeNS(
      ReqIFUnparser.XMLNS_NAMESPACE,
      "xmlns:xhtml",
      ...
      for (const dataType of reqIfContent.dataTypes) {
        ...
        switch (dataType.kind) {
          ...
          case ReqIFDataTypeKind.ENUMERATION:
            dataTypeElement = doc.createElement(
        ...
        if (
          dataType.kind === ReqIFDataTypeKind.ENUMERATION &&
          "values" in dataType
        ) {
          ...
          if (enumDataType.multiValued) {
            dataTypeElement.setAttribute(
              "MULTI-VALUED",
              String(enumDataType.multiValued),
            );
          }
    ...
    if (
      ...
      for (const specObjectType of reqIfContent.specTypes.specObjectTypes) {
        ...
        specObjectTypeElement.setAttribute(
          "IDENTIFIER",
          specObjectType.identifier,
        );

        if (specObjectType.longName) {
          specObjectTypeElement.setAttribute(
            "LONG-NAME",
            specObjectType.longName,
          );
        }
        ...
        if (specObjectType.lastChange) {
          specObjectTypeElement.setAttribute(
            "LAST-CHANGE",
            specObjectType.lastChange,
          );
        }

        // Add attribute definitions
        if (
          specObjectType.attributeDefinitions &&
          specObjectType.attributeDefinitions.length > 0
        ) {
          ...
          for (const attrDef of specObjectType.attributeDefinitions) {
            ...
            switch (attrDef.type) {
              case "BOOLEAN":
                attrDefElement = doc.createElement(
            ...
            if (attrDef.typeRef) {
              ...
              const typeRefElement = doc.createElement(
                "DATATYPE-DEFINITION-" + attrDef.type + "-REF",
              );
      ...
      for (const specificationType of reqIfContent.specTypes
        .specificationTypes) {
        const specificationTypeElement =
          doc.createElement("SPECIFICATION-TYPE");
        specificationTypeElement.setAttribute(
          "IDENTIFIER",
        ...
        if (specificationType.lastChange) {
          specificationTypeElement.setAttribute(
            "LAST-CHANGE",
            specificationType.lastChange,
          );
        }

        // Add attribute definitions (similar to spec object types)
        if (
          specificationType.attributeDefinitions &&
          specificationType.attributeDefinitions.length > 0
        ) {
          const specAttributesElement = doc.createElement("SPEC-ATTRIBUTES");
          ...
          for (const attrDef of specificationType.attributeDefinitions) {
            const attrDefElement = doc.createElement(
              `ATTRIBUTE-DEFINITION-${attrDef.type}`,
            );
            attrDefElement.setAttribute("IDENTIFIER", attrDef.identifier);
            specAttributesElement.appendChild(attrDefElement);
          }
      ...
      for (const specRelationType of reqIfContent.specTypes.specRelationTypes) {
        ...
        specRelationTypeElement.setAttribute(
          "IDENTIFIER",
      ...
      for (const relationGroupType of reqIfContent.specTypes
        .relationGroupTypes) {
        const relationGroupTypeElement = doc.createElement(
          "RELATION-GROUP-TYPE",
        );
        relationGroupTypeElement.setAttribute(
          "IDENTIFIER",
    ...
    if (reqIfContent.specObjects.length > 0) {
      ...
      for (const specObject of reqIfContent.specObjects) {
        ...
        if (specObject.typeRef) {
          const typeElement = doc.createElement("TYPE");
          const specObjectTypeRefElement = doc.createElement(
        ...
        if (
          specObject.attributeValues &&
          specObject.attributeValues.length > 0
        ) {
          ...
          for (const attrValue of specObject.attributeValues) {
            ...
            switch (attrValue.kind) {
              case ReqIFAttributeValueKind.BOOLEAN:
                attributeValueElement = doc.createElement(
            ...
            if (attrValue.definitionRef) {
              ...
              const defRefElement = doc.createElement(
                `ATTRIBUTE-DEFINITION-${attrValue.kind}-REF`,
              );
            ...
            if (
              ...
              if (
                attrValue.kind === ReqIFAttributeValueKind.BOOLEAN ||
                attrValue.kind === ReqIFAttributeValueKind.INTEGER ||
                attrValue.kind === ReqIFAttributeValueKind.REAL
              ) {
                attributeValueElement.setAttribute(
                  "THE-VALUE",
                  String(attrValue.value),
                );
              } else if (
                attrValue.kind === ReqIFAttributeValueKind.DATE ||
                ...
                attrValue.kind === ReqIFAttributeValueKind.ENUMERATION
              ) {
                ...
                if (enumAttrValue.values && enumAttrValue.values.length > 0) {
                  ...
                  for (const enumValue of enumAttrValue.values) {
                    const enumValueRefElement =
                      doc.createElement("ENUM-VALUE-REF");
                ...
                const xhtmlContent =
                  xhtmlAttrValue.originalValue ||
                  addXhtmlNamespaces(xhtmlAttrValue.value);
                ...
                try {
                  ...
                  const xhtmlNode = doc.importNode(
                    xhtmlDoc.documentElement,
                    true,
                  );
    ...
    if (reqIfContent.specRelations.length > 0) {
      ...
      for (const specRelation of reqIfContent.specRelations) {
        ...
        if (specRelation.lastChange) {
          specRelationElement.setAttribute(
            "LAST-CHANGE",
            specRelation.lastChange,
          );
        }
        ...
        if (specRelation.typeRef) {
          const typeElement = doc.createElement("TYPE");
          const specRelationTypeRefElement = doc.createElement(
    ...
    if (reqIfContent.specifications.length > 0) {
      ...
      for (const specification of reqIfContent.specifications) {
        ...
        specificationElement.setAttribute(
          "IDENTIFIER",
        ...
        if (specification.typeRef) {
          const typeElement = doc.createElement("TYPE");
          const specificationTypeRefElement = doc.createElement(
        ...
        // Add attribute values if present
        if (
          specification.attributeValues &&
          specification.attributeValues.length > 0
        ) {
          const valuesElement = doc.createElement("VALUES");
        ...
        if (specification.children && specification.children.length > 0) {
          ...
          const addSpecHierarchy = (parent: Element, hierarchy: any) => {
            ...
            specHierarchyElement.setAttribute(
              "IDENTIFIER",
            ...
            if (hierarchy.lastChange) {
              specHierarchyElement.setAttribute(
                "LAST-CHANGE",
                hierarchy.lastChange,
              );
            }
            if (hierarchy.isTableInternal !== undefined) {
              specHierarchyElement.setAttribute(
                "IS-TABLE-INTERNAL",
                String(hierarchy.isTableInternal),
              );
            }
    ...
    if (reqIfContent.relationGroups.length > 0) {
      ...
      for (const relationGroup of reqIfContent.relationGroups) {
        ...
        relationGroupElement.setAttribute(
          "IDENTIFIER",
          relationGroup.identifier,
        );

        if (relationGroup.longName) {
          relationGroupElement.setAttribute(
            "LONG-NAME",
            relationGroup.longName,
          );
        }
        ...
        if (relationGroup.typeRef) {
          const typeElement = doc.createElement("TYPE");
          const relationGroupTypeRefElement = doc.createElement(
            "RELATION-GROUP-TYPE-REF",
          );
        ...
        if (relationGroup.sourceSpecificationRef) {
          ...
          const specificationRefElement =
            doc.createElement("SPECIFICATION-REF");
          specificationRefElement.textContent =
            relationGroup.sourceSpecificationRef;
        ...
        if (relationGroup.targetSpecificationRef) {
          ...
          const specificationRefElement =
            doc.createElement("SPECIFICATION-REF");
          specificationRefElement.textContent =
            relationGroup.targetSpecificationRef;
        ...
        if (
          relationGroup.specRelationRefs &&
          relationGroup.specRelationRefs.length > 0
        ) {
   ...
   */
  static async unparseToFile(
    reqIFBundle: ReqIFBundle,
    filePath: string,
  ): Promise<void> {
