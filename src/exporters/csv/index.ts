...
export class CSVExporter {
   ...
   */
  static export(bundle: ReqIFBundle, options: CSVExportOptions = {}): string | Map<string, string> {
   ...
   * @param options Export options
   */
  static async exportToFile(bundle: ReqIFBundle, filePath: string, options: CSVExportOptions = {}): Promise<void> {
   ...
   */
  private static exportSplitSpecifications(bundle: ReqIFBundle, options: CSVExportOptions): Map<string, string> {
   ...
   */
  private static determineFields(bundle: ReqIFBundle, options: CSVExportOptions, specification?: ReqIFSpecification): string[] {
    ...
    const specObjectTypes = bundle.coreContent.reqIfContent.specTypes.specObjectTypes;
   ...
   * @returns CSV string
   */
  private static generateSpecificationCSV(bundle: ReqIFBundle, spec: ReqIFSpecification, fields: string[], options: CSVExportOptions): string {
    let csv = '';
    
    // Process all hierarchy elements
    if (spec.children && spec.children.length > 0) {
      for (const hierarchy of bundle.iterateSpecificationHierarchy(spec)) {
        const row = this.generateHierarchyRow(bundle, hierarchy, spec, fields, options);
        if (row.length > 0) {
  ...
  private static generateHierarchyRow(
    ...
    fieldValues.set('Specification', specification.longName || specification.identifier);
    ...
    if (specObject.typeRef) {
      ...
      fieldValues.set('Type', type ? (type.longName || type.identifier) : specObject.typeRef);
    } else {
   ...
   */
  private static formatCSVRow(row: string[], options: CSVExportOptions): string {
    const { delimiter = ',', quoteChar = '"', newline = '\n' } = { ...DEFAULT_OPTIONS, ...options };
 ...
 */
export function exportToCSV(bundle: ReqIFBundle, options: CSVExportOptions = {}): string | Map<string, string> {
 ...
 * @param options Export options
 */
export async function exportToCSVFile(bundle: ReqIFBundle, filePath: string, options: CSVExportOptions = {}): Promise<void> {
