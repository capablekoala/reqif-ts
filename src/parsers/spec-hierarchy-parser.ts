...
export class SpecHierarchyParser {
  ...
  static parse(element: Element, level: number = 1): ReqIFSpecHierarchy {
    ...
    if (element.tagName !== 'SPEC-HIERARCHY') {
      throw new ReqIFError(`Expected SPEC-HIERARCHY element, got ${element.tagName}`);
    }
    
    // Get required attributes
    const identifier = element.getAttribute('IDENTIFIER');
    if (!identifier) {
      throw new ReqIFError('SPEC-HIERARCHY element missing required IDENTIFIER attribute');
    }
    ...
    const isTableInternal = element.getAttribute('IS-TABLE-INTERNAL') === 'true';
    
    // Get SPEC-OBJECT reference
    const specObjectRefElement = element.querySelector('OBJECT > SPEC-OBJECT-REF');
    if (!specObjectRefElement || !specObjectRefElement.textContent) {
      throw new ReqIFError('SPEC-HIERARCHY element missing OBJECT/SPEC-OBJECT-REF');
    }
    ...
    if (childElements.length > 0) {
      for (let i = 0; i < childElements.length; i++) {
        const child = SpecHierarchyParser.parse(childElements[i] as Element, level + 1);
        children.push(child);
      }
    }
  ...
  static unparse(hierarchy: ReqIFSpecHierarchy): string {
    ...
    if (hierarchy.attributeValues && hierarchy.attributeValues.length > 0) {
      result += '  <VALUES>\n';
      result += AttributeValueParser.unparseAttributeValues(hierarchy.attributeValues)
        .split('\n')
        .map(line => `    ${line}`)
        .join('\n');
      result += '\n  </VALUES>\n';
