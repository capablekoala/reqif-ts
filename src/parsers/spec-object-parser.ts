  ...
  static parse(element: Element): ReqIFSpecObject {
    ...
    if (element.tagName !== "SPEC-OBJECT") {
      throw new ReqIFError(
        `Expected SPEC-OBJECT element, got ${element.tagName}`,
      );
    }
    ...
    if (!identifier) {
      throw new ReqIFError(
        "SPEC-OBJECT element missing required IDENTIFIER attribute",
      );
    }
    ...
    if (!typeElement || !typeElement.textContent) {
      throw new ReqIFError(
        "SPEC-OBJECT element missing TYPE/SPEC-OBJECT-TYPE-REF",
      );
    }
