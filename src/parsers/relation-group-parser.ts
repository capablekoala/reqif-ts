...
export class RelationGroupParser {
  ...
  static parse(element: Element): ReqIFRelationGroup {
    ...
    const typeRefElement = element.querySelector(
      "TYPE > RELATION-GROUP-TYPE-REF",
    );
    const typeRef = typeRefElement?.textContent || "";

    // Get source specification reference
    const sourceSpecRefElement = element.querySelector(
      "SOURCE-SPECIFICATION > SPECIFICATION-REF",
    );
    const sourceSpecificationRef =
      sourceSpecRefElement?.textContent || undefined;

    // Get target specification reference
    const targetSpecRefElement = element.querySelector(
      "TARGET-SPECIFICATION > SPECIFICATION-REF",
    );
    const targetSpecificationRef =
      targetSpecRefElement?.textContent || undefined;
    ...
    if (relationsElement) {
      const relationRefElements =
        relationsElement.querySelectorAll("SPEC-RELATION-REF");
