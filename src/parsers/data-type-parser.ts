...
export class DataTypeParser {
  ...
  static parse(element: Element): ReqIFDataType {
    ...
    if (!identifier) {
      throw new Error(
        `${tagName} element missing required IDENTIFIER attribute`,
      );
    }
    ...
    switch (tagName) {
      case "DATATYPE-DEFINITION-BOOLEAN":
        return new ReqIFDataTypeBoolean(
          identifier,
          longName || undefined,
      ...
      case "DATATYPE-DEFINITION-ENUMERATION":
        ...
        for (let i = 0; i < enumValueElements.length; i++) {
          ...
          if (!enumIdentifier) {
            throw new Error(
              "ENUM-VALUE element missing required IDENTIFIER attribute",
            );
          }
          ...
          if (!keyStr) {
            throw new Error(
              "ENUM-VALUE element missing required KEY attribute",
            );
          }
